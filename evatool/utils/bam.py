...
class SRNA(object):
    def __init__(self, stat: Stat, log: Logger, config: Config):
        self.stat = stat
        self.log = log
        self.config = config

    def get_tag_flag(self):
        cmd = ["tail", "-n", "1", self.stat.tagfile]
        ...
        if fields[0] == "ok":
            ...
            self.log.log(message="The length of tags were out of the range 15-40 nt, please check the data.")
            sys.exit()
