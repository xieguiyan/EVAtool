...
def get_analysis_option():
    my_cmdparser = argparse.ArgumentParser(description="transform fq reads to tag with numbers\n")
    ...
    configure = {
        i: getattr(options, i) for i in dir(options) if (i[0] != "_" and i not in ("ensure_value", "read_file", "read_module"))
    }
    return configure
...
def fq2tag(configure):
    ...
    with open(configure["outfile"] + ".freq.stat", "w") as of:
        ...
        of.write(
            "{0}\t{1}\t{2:d}\t{3:.2f}\t{4:d}\n".format(
                flag, "\t".join([str("{0:.2f}".format(j)) for j in len_stat_lst]), out_reads_n, out_reads_n / reads_n, reads_n
            )
        )
