...
def generate_command(script, srafiles, workdir, sample, config_file, config_env):
    ...
    for srafile in srafiles.strip().split(','):
        ...
        println("{fastqdump} {quals} --gzip --split-files {srafile} 1>{sra}.dump.log 2>&1".format(fastqdump=config_env['fastqdump'], quals=quals, srafile=srafile, sra=sra), script)
        ...
        println("java -jar -Xms8000m -Xmx8000m {trimmomatic} SE -threads {cpu_number} {sra_reads} {out_reads} {filter_params} -trimlog {out_reads}.log 1>{out_reads}_run.log 2>&1".format(trimmomatic = config_env['trimmomatic'], cpu_number = config_env['cpu_number'], sra_reads = sra_reads, out_reads = sra_filter_reads_1, filter_params = filter_params), script)
        println("java -jar -Xms8000m -Xmx8000m {trimmomatic} SE -threads {cpu_number} {sra_reads} {out_reads} {filter_params} -trimlog {out_reads}.log 1>{out_reads}_run.log 2>&1".format(trimmomatic = config_env['trimmomatic'], cpu_number = config_env['cpu_number'], sra_reads = sra_filter_reads_1, out_reads = sra_filter_reads_2, filter_params = filter_params), script)
    else:
        ...
        println("{python2} {fq2tag} -i {sras} -c {tag_cut} -o {sample}.fa && echo 'reads2tag translation completed ' >> benchmarks.log && date >> benchmarks.log".format(python2 = config_env['python2'], fq2tag = config_env['fq2tag'], sras = ','.join(sra_lst), tag_cut = config_env['tag_cut'], sample = sample), script)
        ...
        println("{python3} {tag_anno_exp} -i {sample} -t {tag_fa} -o {tempdir} -c {config_file} -l {sample}.anno.exp.log -s {tag_fa}.freq.stat && echo 'tag annotation completed ' >> benchmarks.log && date >> benchmarks.log".format(python3 = config_env['python3'], tag_anno_exp = config_env['tag_anno_exp'], tempdir = tempdir, sample = sample, tag_fa = tag_fa_file, config_file = config_file), script)
